ccLR.grid <- function(cancer = c("breast", "ovarian", "custom"),
  ...
  if (!is.numeric(grid) || any(grid <= 0)) {
    warning(
      "Argument grid must be a numeric vector with all values >0. ",
      "Grid was reset to 1 to run the standard ccLR at the gene-level penetrance."
    )
    grid <- 1
  }
  ...
  for (i in 2:colnum) {
    ...
    if(nrow(df1)>0){
      ...
      if (geno_notation == "n") {
        ...
        df1[,2][df1[,2] == 2] <- 1 # make homos be hets
        carriers <- df1[df1[, list[[1]][i]] == 1, ]
      } else {
        if (geno_notation == "n/n") {
          MAF_cases <- ( (nrow(df1[df1$status == 1 & df1[,2] == "0/1",])) + (2*nrow(df1[df1$status == 1 & df1[,2] == "1/1",])) ) / (2*nrow(allcases))
          MAF_controls <- ( (nrow(df1[df1$status == 0 & df1[,2] == "0/1",])) + (2*nrow(df1[df1$status == 0 & df1[,2] == "1/1",])) ) / (2*nrow(allcontrols))
          df1[,2][df1[,2] == "1/1"] <- "0/1" # make homos be hets
          carriers <- df1[df1[, list[[1]][i]] == "0/1", ]
        }
      }
      ...
      if(nrow(carriers)>0){
        ...
        for (s in grid) {
          for (h in seq_along(strat)) {
            ...
            h_carrier <- inc_rates$IR * (((gene_rates$RR - 1) * s) + 1)
            cumhazard_carrier <- cumsum(h_carrier)
            
            ## m2$RR <- gene_rates$RR[ match(m2$age_pen, gene_rates$Age) ]
            
            ##new here
            RR_scaled <- (((gene_rates$RR - 1) * s) + 1)
            m2$RR <- RR_scaled[ match(m2$age_pen, gene_rates$Age) ]
            ## 
            ...
            if (geno_notation == "n") {
              K <- nrow(df2[which(df2[,2]==1),])
            } else {
              if (geno_notation == "n/n") {
                K <- nrow(df2[which(df2[,2]== "0/1"),])
              }
            }
            ...
            if (geno_notation == "n") {
              ...
              if (geno_notation == "n/n") {
                m3 <- m2[which(m2[, 2] =="0/1"), ]
              }
            }
            
            if (K > 0) {
              log_prod_likelihood <- sum(log(m3$Likelihood))
              log_NK <- K * log(N)
              log_sum_denom <- K * log(sum(m2$Likelihood))
              log_LR <- log_NK + log_prod_likelihood - log_sum_denom
              LR <- exp(log_LR)
            } else {
              LR <- 1
            }
            
            if (is.null(LR_scaled_results[[as.character(s)]])) {
              LR_scaled_results[[as.character(s)]] <- c()
            }
            LR_scaled_results[[as.character(s)]][strat[h]] <- LR
        ...
        # Calculates final LR per grid parameter (product of stratified LRs)
        finalLR_all <- sapply(LR_scaled_results, function(stratum_LRs) prod(unlist(stratum_LRs), na.rm = TRUE))
        
        ## Bayes factor
        lr_vals <- as.numeric(finalLR_all)
        
        
        # Store all LRs at all grid parameters
        for (s_val in names(finalLR_all)) {
          colname <- paste0("LR at s=", s_val)
          results[variant, colname] <- finalLR_all[[s_val]]
        }
        
        
        ### Get LR at s=1 (standard ccLR)
        LRs1 <- finalLR_all[names(finalLR_all) == 1]
        
        if (identical(LRs1, "NaN")) {
          results[variant,"LR at s=1 ACMG/AMP Evidence"] <- "-"
        } else {
          if( is.na(LRs1) ) {
            results[variant,"LR at s=1"] <- "-"
            results[variant,"LR at s=1 ACMG/AMP Evidence"] <- "-"
          } else {
            results[variant,"LR at s=1"] <- LRs1
            if (LRs1 >= 350) results[variant,"LR at s=1 ACMG/AMP Evidence"] <- "Pathogenic Very Strong"
            if (LRs1 >= 18.7 & LRs1 < 350) results[variant,"LR at s=1 ACMG/AMP Evidence"] <- "Pathogenic Strong"
            if (LRs1 >= 4.33 & LRs1 < 18.7) results[variant,"LR at s=1 ACMG/AMP Evidence"] <- "Pathogenic Moderate"
            if (LRs1 >= 2.08 & LRs1 < 4.33) results[variant,"LR at s=1 ACMG/AMP Evidence"] <- "Pathogenic Supporting"
            if (LRs1 <= 0.0029) results[variant,"LR at s=1 ACMG/AMP Evidence"] <- "Bening Very Strong"
            if (LRs1 > 0.0029 & LRs1 <= 0.053) results[variant,"LR at s=1 ACMG/AMP Evidence"] <- "Bening Strong"
            if (LRs1 > 0.053 & LRs1 <= 0.231) results[variant,"LR at s=1 ACMG/AMP Evidence"] <- "Bening Moderate"
            if (LRs1 > 0.231 & LRs1 <= 0.48) results[variant,"LR at s=1 ACMG/AMP Evidence"] <- "Bening Supporting"
            if (LRs1 < 2.08 & LRs1 > 0.48) results[variant,"LR at s=1 ACMG/AMP Evidence"] <- "No Evidence"
          }
        }
        
        
        # Get max LR and associated s
        maxs_pos <- which.max(finalLR_all)
        maxs <- as.numeric(names(finalLR_all)[maxs_pos])
        maxLR <- finalLR_all[maxs_pos]
        
        # ACMG classification based only on LR thresholds, regardless of grid parameter
        if (is.na(maxLR)) {
          acmg_code <- "-"
          maxLR <- "-"
        } else if (maxLR >= 350) {
          acmg_code <- "Pathogenic Very Strong"
        } else if (maxLR >= 18.7) {
          acmg_code <- "Pathogenic Strong"
        } else if (maxLR >= 4.33) {
          acmg_code <- "Pathogenic Moderate"
        } else if (maxLR >= 2.08) {
          acmg_code <- "Pathogenic Supporting"
        } else if (maxLR <= 0.0029) {
          acmg_code <- "Benign Very Strong"
        } else if (maxLR <= 0.053) {
          acmg_code <- "Benign Strong"
        } else if (maxLR <= 0.231) {
          acmg_code <- "Benign Moderate"
        } else if (maxLR <= 0.48) {
          acmg_code <- "Benign Supporting"
        } else {
          acmg_code <- "No Evidence"
        }
        
        results[variant, "Max LR"] <- maxLR
        results[variant, "Max LR ACMG/AMP Evidence"] <- acmg_code
        results[variant, "Grid parameter at Max LR"] <- maxs
        ...
        if (progress == TRUE) {
          cat("Variant", i-1, "out of", colnum-1, "skipped (no carriers):", variant, "\n")
        }
      } 
    } else {
      if (progress == TRUE) {
